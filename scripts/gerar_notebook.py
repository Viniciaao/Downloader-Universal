...
celulas = [
    ...
    code(
        "# ⚙️ 1. SETUP — instale tudo (rode só uma vez por sessão)\n",
        "!test -d Downloader-Universal && (cd Downloader-Universal && git pull -q) || git clone -q https://github.com/Viniciaao/Downloader-Universal.git\n",
        "%cd /content/Downloader-Universal\n",
    ...
    md(
        ...
        "| DDownload (ex-dll.to) | ✅ | extrator próprio: API pública + fluxo XFS (contador, esperas, redirect final). Em IP de datacenter (Colab/VPS) o Cloudflare Turnstile pode barrar a automação — em IP residencial funciona |\n",
